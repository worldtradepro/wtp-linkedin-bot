...
function patchFlash(body) {
  ...
  swap('tiers: tierCounts(pool), picks: picks };',
    'tiers: tierCounts(pool), lanes: flashLanes(), picks: (window.__WTP_FLASH = picks.map(function(it) { return { url: it.source_url, title: it.project_name, country: it.country, sector: it.sector }; }), picks) };');
